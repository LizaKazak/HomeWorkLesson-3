...
1. **git push** - команда, которая отправляет изменения, сделанные в локальном репозитории в GitHub, в удалённый репозиторий.
2. **git push --set-upstream origin BranchName**, где **BranchName** - имя ветки, с которой отправляются изменения, командой **set-upstream** мы показываем, что ветка **BranchName** доступна, открыта и готова для загрузки на GitHub, что она создана и зафиксирована. Данная команда вводится единожды, для знакомства GitHub с этой веткой. В дальнейшем мы пушим с помощью **git push**.
3. **git remote add origin ссылка** - команда, с помощью которой мы добавляем, привязываем, подключаем ссылку на удаленный репозиторий к локальному и устанавливаем их взаимосвязь.
4. **git pull** - команда, с помощью которой мы сливаем все изменения из удалённого репозитория в локальный, устанавливая их аутентичность.

## Итоги семинаров
На семинарах разобрана и отработана только часть команд и особенностей работы в Git.
Очень желательно найти время и отработать задания в Git-тренажёре **Lean Git Branching**.
Павел, Вам - бесконечное спасибо!
